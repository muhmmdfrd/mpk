...
<?php
  ...
  $find_datas = "SELECT week(tgl_kirim),COUNT(pesan_id) from tb_pesan WHERE week(tgl_kirim) < week(now()) + 1 AND year(tgl_kirim) = year(now()) GROUP BY week(tgl_kirim) limit 8" ;
  ...
  while ($row = $stat->fetch(PDO::FETCH_ASSOC) ) {
    for($i; $i < 8; $i++){
      ...
      if(intval($weekNumber + $i) == $row['week(tgl_kirim)']){
        $chart_datas[$i] = $row['COUNT(pesan_id)'];
        $i++;break;
      }else{
        $chart_datas[$i] = 0;
      }
...
<body onload="convert_array_php_to_js();index_aset('index_pesan','this_month');">
  <div class='container-fluid'>
    <div class='col-lg-12'>
      <div class='konten'>
        <h4>Diagram Pesan Siswa</h4>
    ...
    <div class='col-lg-4 mobile-slide' id='box-filter1'>
      <div class='konten' onclick=" index_aset('index_pesan','this_month');">
        <h4><b>Pesan Siswa Bulan ini</b></h4>
        <?php echo get_month(date('m')); ?>
      </div>
    </div>
    <div class='col-lg-4 mobile-slide' id='box-filter2'>
      <div class='konten' onclick=" index_aset('index_pesan','all');">
        <h4><b>Semua Pesan Siswa</b></h4>
        <?php echo date('Y-m-d'); ?>
      </div>
    </div>
    <div class='col-lg-4 mobile-slide' id='box-filter3'>
      <div class='konten' onclick="show_filter();">
        <h4><b>Cari Pesan Siswa</b></h4>
        <span>Nathieq</span>
      </div>
    </div>
